...
- **Tech Stack:** Python 3.14+, Loguru, Rich, Pytest, Ruff, Pyright, MkDocs, MkDocs-Material
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/media_downloader/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/media_downloader/` – Main application code
  - `src/media_downloader/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/media_downloader/.env`.
- Secrets should not be committed.

## Related

- `src/media_downloader/config/media_downloader_paths.py`
