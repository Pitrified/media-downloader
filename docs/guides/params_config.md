...
|---|---|---|
| **Config** | `src/media_downloader/config/` | Defines the *shape* of settings as Pydantic models |
| **Params** | `src/media_downloader/params/` | Loads *actual values* and produces a Config via `to_config()` |
...
- [`SampleConfig`](../../reference/media_downloader/config/sample_config/) - config side
- [`SampleParams`](../../reference/media_downloader/params/sample_params/) - params side
...
1. **Single constructor argument**: `env_type: EnvType | None = None`.
   Pass it explicitly from the caller (e.g., `MediaDownloaderParams`).  When
...
from media_downloader.params.env_type import (
    EnvLocationType, EnvStageType, EnvType,
    UnknownEnvLocationError, UnknownEnvStageError,
)
from media_downloader.config.sample_config import NestedModel, SampleConfig
...
`MediaDownloaderParams` (the top-level singleton) owns all Params instances
...
```python
from media_downloader.params.env_type import EnvLocationType, EnvStageType, EnvType
