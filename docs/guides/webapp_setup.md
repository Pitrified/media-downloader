...
1. Configure the following:
   - **Name**: Your app name (e.g., "Media Downloader Dev")
...
Create a `.env` file at `~/cred/media_downloader/.env`:
...
uv run uvicorn media_downloader.webapp.app:app --reload

# Or using the app module
uv run python -m media_downloader.webapp.app
```
...
using the [cdn_load.sh](https://github.com/Pitrified/media-downloader/blob/main/scripts/webapp/cdn_load.sh) script.
...
3. Configure:
   ...
   - **Start Command**: `uvicorn media_downloader.webapp.app:app --host 0.0.0.0 --port $PORT`
...
1. Create a new router in `src/media_downloader/webapp/routers/`
2. Include it in `main.py`:
   ```python
   from media_downloader.webapp.routers import new_router
