# First tasks for media_downloader
...
- Replace the generic project description with a real description of what media_downloader does.
...
If **no**, remove:
- `src/media_downloader/webapp/`
- `src/media_downloader/config/webapp/`
...
If **no**, remove those from `src/media_downloader/config/webapp/` and the webapp routers/middleware.
...
Replace the sample environment variable names with the real ones needed by media_downloader.
Remove any variables that are no longer relevant.

## Update `.github/agents/`

Review each agent definition file. Either:
- Update the agent description and commands to reflect media_downloader, or
...
that are not relevant to media_downloader. Keep only what will be actively used.

## Update `README.md`

Replace the template boilerplate with a real description of media_downloader:
