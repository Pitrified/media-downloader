...
``src/media_downloader/config/sample_config.py``.

Config / Params split:

* **Config** (``src/media_downloader/config/``) defines only the *shape* of
...
See Also:
    ``SampleConfig`` - the paired config model in ``src/media_downloader/config/``.
...
from media_downloader.config.sample_config import NestedModel
from media_downloader.config.sample_config import SampleConfig
